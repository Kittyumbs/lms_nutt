...
import { collection, onSnapshot, updateDoc, deleteDoc, doc, query, orderBy, setDoc, getDocs, where, documentId, deleteField, serverTimestamp } from "firebase/firestore"; // Import deleteField
export const useKanbanBoard = () => {
    const [columns, setColumns] = useState(initialColumns);
    const existingTicketIds = useRef(new Set()); // Store existing 5-digit IDs
    useEffect(() => {
        // Query to fetch tickets that are not archived, ordered by creation date
        const q = query(collection(db, "tickets"), where("archived", "==", false), // Filter out archived tickets
    ...
    const generateUnique5DigitId = useCallback(async () => {
        let newId;
        let isUnique = false;
        let attempts = 0;
    ...
    const moveTicket = useCallback(async (ticketId, newStatus) => {
        ...
        const base = { status: newStatus };
        const patch = newStatus === 'done'
