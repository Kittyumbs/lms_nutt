      ...
      className={({ isActive }) =>
        collapsed
          ? `flex justify-center items-center h-12 mx-1 rounded-lg text-xl transition-colors duration-200 ${
              isActive ? 'bg-[#057EC8] text-white shadow-sm' : 'text-[#333] hover:bg-[#f0f8ff] hover:text-[#057EC8]'
            }`
          : `flex items-center px-3 py-2 mx-2 rounded-lg text-sm transition-colors duration-200 ${
              isActive ? 'bg-[#057EC8] text-white font-semibold shadow-sm' : 'text-[#333] hover:bg-[#f0f8ff] hover:text-[#057EC8]'
            }`
      }
      title={label}
      end
  ...
  return (
    <>
      ...
      >
        ...
        >
          {!collapsed && <div style={{ fontWeight: 700, fontSize: '18px', letterSpacing: '0.5px' }}>TaskManage</div>}
          <button
